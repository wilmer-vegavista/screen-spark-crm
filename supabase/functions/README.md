...
- **Statement timeout:** the page reads the views through PostgREST with the admin's session,
  ...
  (`20260909120100_fortnox_cashflow_perf.sql`); on the dev project's 1 292 postings each
  answers in under 100 ms. If a live ledger ever grows past what that allows, the fix is a
