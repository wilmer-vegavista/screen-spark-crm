-- Telefonnummer matchas nu på de sista 8 siffrorna i stället för hela
-- sifferföljden, så att landskod och riktnummerformat inte spelar roll:
-- +46 70 123 45 67, 070-123 45 67 och 0046701234567 räknas som samma nummer.
...
AS $$
  ...
  mine AS (
    ...
    WHERE owner_id = auth.uid()
      AND ((kind = 'telefon' AND full_len >= 7)
...
CREATE OR REPLACE FUNCTION public.get_all_list_duplicates()
RETURNS TABLE(list_id uuid, match_type text, match_value text, other_party text, source text)
LANGUAGE sql
STABLE
SECURITY DEFINER
SET search_path = public
AS $$
  ...
  list_vals AS (
    SELECT cols.list_id, cols.owner_id, cols.kind,
           CASE cols.kind
             WHEN 'telefon' THEN right(regexp_replace(coalesce(r.data->>cols.col_id, ''), '\D', '', 'g'), 8)
             ELSE lower(btrim(coalesce(r.data->>cols.col_id, '')))
           END AS norm,
  ...
  SELECT DISTINCT mine.list_id, mine.kind, mine.raw,
         coalesce(p.full_name, p.email, 'Okänd säljare'),
         o.source
